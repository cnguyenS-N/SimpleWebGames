...
- **Tic-Tac-Toe** (`tic-tac-toe.html`, `game/game.html`, `game.js`, `tic-tac-toe.css`)
...
For a local web server, run one of the following from the repository root and open the shown localhost URL. Game pages can also be opened directly at their paths listed above.
...
|   `-- README.md       # Tic-Tac-Toe play screen notes
...
- Games are plain HTML/CSS/JS with no build step required for normal development.
- Tic-Tac-Toe uses a root landing page, a `game/` play screen, and root-level shared logic/styles.
- Air Hockey, Connect 4, and Memory Match keep their HTML/CSS/JS files in game-specific folders.
